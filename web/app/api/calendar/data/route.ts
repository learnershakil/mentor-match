...
export async function GET(req: NextRequest) {
  try {
    ...
    // Fetch the user's interest
    const user = await prisma.user.findUnique({
      where: { id: session.user.id },
      select: { intrest: true, role: true },
    });
    ...
    // Create interest filter based on query param or user's interest
    const interest = interestFilter || user.intrest;
    ...
    if (session.user.role === "MENTOR") {
      ...
      // If interest filter is applied, filter assignments by student interest
      const filteredAssignments = interest
        ? assignments.filter((a) => a.student.user.intrest === interest)
        : assignments;
      ...
      // If interest filter is applied, filter events by student interest
      const filteredEvents = interest
        ? events.filter((e) => e.student.user.intrest === interest)
        : events;
      ...
      const formattedAssignments = filteredAssignments.map((assignment) => ({
        ...
        type: "deadline",
        category: assignment.student.user.intrest,
      ...
      const formattedEvents = filteredEvents.map((event) => ({
        ...
        category: event.student.user.intrest,
      ...
      // Fetch mentor meetings relevant to the student's interest
      const mentorMeetings = await prisma.mentorMeeting.findMany({
        where: {
          ...dateFilter,
          category: interest,
      ...
      const formattedMeetings = mentorMeetings.map((meeting) => ({
        ...
        mentorName: meeting.mentorship?.user
          ? `${meeting.mentorship.user.firstName} ${meeting.mentorship.user.lastName}`
          : "Unknown Mentor",
      ...
      const formattedEvents = events.map((event) => ({
        ...
        mentorName: `${event.mentor.user.firstName} ${event.mentor.user.lastName}`,
      }));
